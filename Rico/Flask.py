...
socketio = SocketIO(app, cors_allowed_origins="*")  # allow cross-origin for local testing
...
<body>
  ...
  <div class="row" id="controls">
    ...
    <form id="simForm" style="display:flex; gap:8px; align-items:center; flex-wrap:wrap;">
      <label>Altitude (m): <input id="sim_alt" type="number" step="0.1" required></label>
      <label>Speed (m/s): <input id="sim_spd" type="number" step="0.1" required></label>
      <label>Lat: <input id="sim_lat" type="number" step="0.000001" required></label>
      <label>Lon: <input id="sim_lon" type="number" step="0.000001" required></label>
  ...
  <div class="row">
    <canvas id="altitude" width="1000" height="200"></canvas>
  </div>

  <!-- load socket.io first (CDN is fine for socket.io) -->
  <script src="https://cdn.socket.io/4.7.2/socket.io.min.js"></script>

  <!-- Three.js as ES modules (local files) -->
  <script type="module">
    ...
    const hemi = new THREE.HemisphereLight(0xffffff, 0x080820, 1.0);
    scene.add(hemi);
    ...
    let drone = null;
    loader.load("/static/models/dji_mavic_air.glb",
      (gltf) => {
        drone = gltf.scene;
    ...
    form.addEventListener("submit", async (ev) => {
      ...
      try {
        ...
        // we also emit locally so UI updates immediately
        socket.emit("manual_update", payload);
      } catch (err) {
        console.error("Failed to POST simulate:", err);
      }
    });
...
def simulate():
    ...
    # Broadcast manual update to connected clients immediately
...
def zmq_listener():
    ...
    while True:
        ...
        # message expected: {"id": "...", "x":..., "y":..., "z":..., "vx":..., "vy":..., "vz":...}
        ...
        with telemetry_lock:
            ...
            telemetry_data["speed"] = float(message.get("speed",
                                            (message.get("vx",0.0)**2 + message.get("vy",0.0)**2 + message.get("vz",0.0)**2) ** 0.5))
